...
// ==================== TAB LOGGING (for debugging) ====================
// Logs tab creation to chrome.storage.local for post-crash debugging
let totalTabsCreated = 0;
let tabCreationLog = []; // Log every tab creation for debugging

/**
 * Log tab creation for debugging - persists to chrome.storage.local
 */
async function logTabCreation(tabId, url, poolSize) {
  const entry = {
    timestamp: new Date().toISOString(),
    tabId,
    url,
    poolSize,
    totalCreated: totalTabsCreated
  };
  tabCreationLog.push(entry);

  // Persist to storage so logs survive crashes
  try {
    await chrome.storage.local.set({
      tabCreationLog: tabCreationLog.slice(-100), // Keep last 100 entries
      lastTabCreation: entry
    });
  } catch (e) {
    console.error('[TabFetcher] Failed to persist tab log:', e);
  }
}

// Reset tab counters (call this when starting a new crawl)
...
async function acquireCrawlTab(useIncognito = false) {
  ...
  for (const entry of crawlTabPool) {
    if (!entry.inUse) {
      ...
      try {
        ...
        entry.inUse = true;
        console.log(`[TabFetcher] Acquired existing tab from pool: ${entry.tabId} (pool: ${crawlTabPool.length}, reusing)`);
        return entry;
      } catch (e) {
  ...
  // Log this tab creation
  await logTabCreation(tab.id, 'about:blank', crawlTabPool.length + 1);
 ...
 *
 * IMPORTANT: Some pages never reach 'complete' status due to:
 * - Long-running websocket/SSE connections
 * - Analytics scripts with persistent connections
 * - Streaming responses that never "finish"
 *
 * To handle this, we use a shorter timeout (10s) and proceed gracefully
 * if the tab is still loading. The content detection logic (DOM stability,
 * network idle, content plateau) will correctly identify when content is ready.
 */
function waitForTabComplete(tabId, timeout) {
  // Don't wait forever for 'complete' - some pages never reach it
  const MAX_WAIT_FOR_COMPLETE = 10000; // 10 seconds max
  const effectiveTimeout = Math.min(timeout, MAX_WAIT_FOR_COMPLETE);

  return new Promise((resolve) => {  // Always resolves, never rejects on timeout
    const timeoutId = setTimeout(() => {
      cleanup();
      console.log(`[TabFetcher] Tab ${tabId} still loading after ${effectiveTimeout}ms, proceeding to content detection`);
      resolve();  // Proceed anyway - content detection will handle readiness
    }, effectiveTimeout);
    ...
    chrome.tabs.get(tabId).then(tab => {
      ...
      // If tab check fails, proceed anyway - content detection will handle it
      cleanup();
      resolve();
    });
  });
  ...
  const result = await chrome.scripting.executeScript({
    ...
    func: () => {
      ...
      function extractLinks() {
        ...
        // 10. SPA route discovery via clicking - DISABLED
        // This feature was causing tabs to open uncontrollably when clicking on
        // elements triggered window.open() or target="_blank" links (social buttons, etc.)
        // TODO: Re-enable with proper safeguards (intercept window.open, check target attr)
        console.log(`[TabFetcher] SPA click discovery disabled for safety`);

        // Restore original history methods (still needed for cleanup)
        history.pushState = originalPushState;
        history.replaceState = originalReplaceState;

        console.log(`[TabFetcher] Extracted ${links.size} links using DOM APIs`);
